...
class weighted_digraph:
    ...
    def dijkstra(self, start):
        ...
        while todo:
            ...
            minDistNode = min(todo, key = lambda node: node.distance)
            todo.remove(minDistNode)

            # append its distance and value to the result list
            #result.append([minDistNode.distance, minDistNode.value])

            if not  track_previous or not minDistNode.previous:
                result.append([minDistNode.distance, minDistNode.value])
            else:
                tmp = minDistNode.previous
                tracked_result = [minDistNode.distance, minDistNode.value]
                while tmp:
                    tracked_result.append(tmp.value)
                    tmp = tmp.previous
                result.append(tracked_result)
            ...
            # for each of the edges in the minimum node
                ...
                # existing distance
                    ...
                    # todo list
            for edge in minDistNode.edges:
                temp = minDistNode.distance + edge.weight
                if temp < edge.to_node.distance:
                    edge.to_node.distance = temp
                    edge.to_node.previous = minDistNode
